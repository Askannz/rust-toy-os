...
impl SystemClock {
    pub fn new(runtime_services: &RuntimeServices) -> Self {
        ...
        SystemClock { period_s, epoch_offset }
    }
    ...
    fn get_epoch_time(runtime_services: &RuntimeServices) -> f64 {
        ...
        let t_chrono = Utc.from_local_datetime(
            &NaiveDate::from_ymd_opt(
                t_uefi.year().into(), t_uefi.month().into(), t_uefi.day().into()
            )
            .expect("Invalid RTC clock")
            .and_hms_opt(
                t_uefi.hour().into(), t_uefi.minute().into(), t_uefi.second().into()
            )
            .expect("Invalid RTC clock")
        ).unwrap();

        let secs_since_epoch: u64 = 
            (t_chrono - DateTime::UNIX_EPOCH)
            .num_seconds().try_into()
